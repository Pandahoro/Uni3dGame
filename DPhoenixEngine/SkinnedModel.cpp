	...
	const std::string& texturePath, bool flipTexY)
{
	...
	//initialise counters for vertex / bone loading
	...
	std::map<UINT, std::vector<VertexWeight>>& vertToBoneWeights)
{
	...
	for (int i = 0; i < mesh->mNumBones; i++)
	{
		...
		for (int j = 0; j < bone->mNumWeights; j++)
		{
			...
			//UPDATE: The vertex id needs to be updated from teh overall count at this point
			//so that transforms are correctly mapped!
	...
	//so that transforms are correctly identified
	...
	bool flipTexY, std::vector<PosNormalTexTanSkinned>& _vertices)
{
	...
	//UPDATE: Keep a running total of the vertices for each subset / submesh
	//to ensure the vertices are correctly weighted to the right bones
...
//NEW METHODS FOR NAVIGATING ANIMATIONS
